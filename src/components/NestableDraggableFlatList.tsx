...
function NestableDraggableFlatListInner<T>(
  ...
  const { scrollableRef, setOuterScrollEnabled } = useSafeNestableScrollContainerContext();
  ...
  const onListContainerLayout = useStableCallback(async ({ containerRef }) => {
    ...
    containerRef.current.measureLayout(scrollableRef.current, onSuccess, onFail);
  });
